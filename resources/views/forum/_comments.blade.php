...
<div id="comment" style="position: relative;">
            ...
            <div class="comment-forum p-4 ">
                <div class="d-flex justify-content-between mb-1 ">
                    <div class="profile-forum">
                        <img src="
                     @if ($forum->stat_pengirim == 'member data complete' &&
                        $forum->jenis_kelamin == 'L')
                        {{ asset('assets/img/member_l.png') }}
                    @elseif($forum->stat_pengirim=='member data complete' && $forum->jenis_kelamin=='P')
                        {{ asset('assets/img/member_p.png') }}
                    @elseif($forum->stat_pengirim=='data complete' && $forum->jenis_kelamin=='L')
                        {{ asset('assets/img/non_member_l.png') }}
                    @elseif($forum->stat_pengirim=='data complete' && $forum->jenis_kelamin=='P')
                        {{ asset('assets/img/non_member_p.png') }}
                    @elseif($forum->stat_pengirim=='new user')
    ...
    <div class="d-flex flex-column mt-2 ">
        <h6>{{ $forum->nama_pengirim }} 
            @if ($forum->stat_pengirim == 'admin-mentor')
                <i class="fas fa-check-circle"></i>
            @elseif($forum->stat_pengirim == 'member data complete')
...
<div class="dropdown show hamburger-dot ">
    ...
    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="delete">
        @if ($forum->nama_pengirim == $account->nama)
            <a class="dropdown-item d-flex text-center" href="{{ route('forum.delete', $forum->post_uuid) }}">
...
<div class="d-flex mt-3 like-comment ">
    @if ($forum->user_like == 'True')
        <a href="{{ route('forum.unlike_post', $forum->post_uuid) }}" style="text-decoration: none"
            class="btn">
...
<nav aria-label="Page navigation example pt-5">
    <ul class="pagination pagination-forum justify-content-center">
        <li class="page-item"><a class="page-link page-prev-next" href="{{ route('forum.index') }}"
                data-page="{{ $page - 1 > 0 ? $page - 1 : 1 }}">Prev</a>
        </li>
        ...
        @endforeach
        <li class="page-item"><a class="page-link page-prev-next" href="{{ route('forum.index') }}"
                data-page="{{ $page + 1 < $total_page_forum ? $page + 1 : $total_page_forum }}">Next</a>
        </li>
    </ul>
</nav>
<!-- End Pagination -->
