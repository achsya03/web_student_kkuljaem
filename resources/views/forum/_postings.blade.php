...
<div id="post">
            ...
            <div class="comment-forum p-4 ">
                <div class="d-flex justify-content-between mb-1 ">
                    <div class="profile-forum">
                        <img src="
                     @if ($index->stat_pengirim == 'member data complete' &&
                        $index->jenis_kelamin == 'L')
                        {{ asset('assets/img/member_l.png') }}
                    @elseif($index->stat_pengirim=='member data complete' && $index->jenis_kelamin=='P')
                        {{ asset('assets/img/member_p.png') }}
                    @elseif($index->stat_pengirim=='data complete' && $index->jenis_kelamin=='L')
                        {{ asset('assets/img/non_member_l.png') }}
                    @elseif($index->stat_pengirim=='data complete' && $index->jenis_kelamin=='P')
                        {{ asset('assets/img/non_member_p.png') }}
                    @elseif($index->stat_pengirim=='new user')
    ...
    <div class="d-flex flex-column mt-2 ">
        <h6>{{ $index->nama_pengirim }}
            @if ($index->stat_pengirim == 'admin-mentor')
                <i class="fas fa-check-circle"></i>
            @elseif($index->stat_pengirim == 'member data complete')
...
<div class="dropdown hamburger-dot ">
    <a class="btn dropdown" href="#" id="deletedropdowm" role="button" data-toggle="dropdown" aria-haspopup="true"
        aria-expanded="false">
    ...
    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="delete">
        @if ($index->nama_pengirim == $account->nama)
            <a class="dropdown-item d-flex text-center" href="{{ route('forum.delete', $index->post_uuid) }}">
...
<div class="d-flex mt-3 like-comment ">
    @if ($index->user_like == 'True')
        <a href="{{ route('forum.unlike_post', $index->post_uuid) }}" style="text-decoration: none"
            class="btn">
            <i class="fas fa-heart active"></i>
        </a>
    @elseif($index->user_like=='False')
        <a href="{{ route('forum.like_post', $index->post_uuid) }}" style="text-decoration: none"
            class="btn">
